...
INVENTORY_HINT = (
    "Set `inventory` to the URL your `objects.inv` is served under "
    "(`https://<domain>/page/` if you host on ReadTheDocs), or to `null` if there is none."
)


def inventory_url(meta: ScverseEcosystemPackages) -> str | None:
    """Where a package's `objects.inv` is served, or `None` where there is none to look for.

    `None` for a package in another language, or one whose `inventory` is explicitly null.
    A set `inventory` is taken as given, an absent one is derived from `documentation_home`:
    ...
    - Otherwise the docs link is assumed to be the documentation root. ReadTheDocs behind a custom
      domain is indistinguishable from that here, so those packages have to point `inventory` at
      their `/page/` themselves.
    """
    ...
    return f"{re.split(r'[#?]', docs_home)[0].rstrip('/')}/objects.inv"
...
class InventoryChecker(HTTPValidator):
    ...
    async def __call__(self, url: str, context: str) -> None:
        """Record the root under which the package's `objects.inv` is served."""
        try:
            response = await self.client.head(url)
        except Exception as e:
            msg = f"inventory:{context}: {url} is not reachable: {e}\n{INVENTORY_HINT}"
            raise ValidationError(msg) from e
        if response.status_code != httpx.codes.OK:
            msg = f"inventory:{context}: No inventory at {url} (error {response.status_code}).\n{INVENTORY_HINT}"
            raise ValidationError(msg)
...
class Checker:
    ...
    def http_checks(self, pkg_id: str, tmp_meta: ScverseEcosystemPackages) -> Generator[Awaitable[None]]:
        ...
        if inv_url := inventory_url(tmp_meta):
            yield self.check_inventory(inv_url, pkg_id)
